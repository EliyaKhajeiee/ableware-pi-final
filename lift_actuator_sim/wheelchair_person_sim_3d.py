...
SLING_T     = 0.018    # kept for back-compat only
...
ACT_SIDE_Y  = 0.193                # Y mount: snug against harness belt plates
ACT_X       = 0.0                  # X mount: beside hips, centred front-back
...
class WheelchairLiftSim3D:
    ...
    # ── Side actuators (behind backrest, shaft → belt) ──────────────────

    def _build_actuators(self):
        """
        Two vertical linear actuators beside the person's hips.

        Modelled on a PA-14 class actuator (1000 N, 4-inch stroke, 12/24 V DC).
        Shaft top connects directly to the harness belt side plates.
        Housing mounts to the wheelchair seat frame via a bracket.

        Each side:
          housing  — fixed outer tube mounted to seat frame (static)
          shaft    — inner rod, animated upward; top bolts to belt plate
          end-cap  — clevis / mounting point at bottom
          plate    — belt attachment bracket at shaft top (animated)
        """
        # Position actuator so shaft top meets belt at rest.
        # Belt Z ≈ SEAT_H + PELVIS_H(0.12) + 0.01 = 0.610
        belt_z              = SEAT_H + 0.12 + 0.01
        shaft_base_z        = belt_z - ACT_SHAFT_H
        body_base_z         = shaft_base_z - ACT_BODY_H
        self._shaft_base_z  = shaft_base_z

        self._housing_ids = []
        self._shaft_ids   = []
        self._plate_ids   = []   # belt attachment plates (animated)

        # Motor housing proportions (like a real PA-14)
        ...
        for sign, y in ((+1, ACT_SIDE_Y), (-1, -ACT_SIDE_Y)):
            ...
            _box([0.022, 0.018, 0.024],
                 [ACT_X, y, body_base_z - 0.024], color=C_CAP)
            _cyl(0.006, 0.042, [ACT_X, y, body_base_z - 0.026],
                 _q(math.pi/2, 0, 0), C_FRAME_LT)

            # Mounting bracket — bolts actuator to seat frame rail
            bkt_y = y + sign * (ACT_R * 1.28 + 0.012)
            _box([0.038, 0.012, 0.050],
                 [ACT_X, bkt_y, motor_cz], color=C_FRAME_DK)
            # Bolt heads
            for dz in (-0.014, +0.014):
                _cyl(0.005, 0.005,
                     [ACT_X, bkt_y + sign * 0.012, motor_cz + dz],
                     _q(math.pi/2, 0, 0), C_FRAME_LT)

            # Wiring / power cable exiting motor body
            cable_y = y - sign * ACT_R * 0.88
            _cyl(0.007, 0.058,
                 [ACT_X, cable_y, body_base_z + 0.032],
            ...
            # ── Belt attachment plate at shaft top (animated) ──────────
            plate = _vbox([0.032, 0.022, 0.010],
                          [ACT_X, y, belt_z], color=C_CAP)
            self._plate_ids.append(plate)

    # ── Seated person + waist harness ────────────────────────────────────

    def _build_person(self):
        """Build the seated person model (delegated to SeatedPerson)."""
        person_seat_z = SEAT_H
        self._person = SeatedPerson(
            seat_z=person_seat_z,
            seat_h=SEAT_H,
            seat_w=SEAT_W,
            seat_d=SEAT_D
        )
    ...
    def _update_visuals(self, ext: float, stalled: bool,
                        ...
                        force_per_act: float):
        # Colour coding — shaft shows operational state
        ...
        shaft_top = self._shaft_base_z + ACT_SHAFT_H + ext
        ...
        for i, (sign, y) in enumerate(((+1, ACT_SIDE_Y), (-1, -ACT_SIDE_Y))):
            ...
            _recolor(self._housing_ids[i], C_ACT_HOUSE)

            # belt attachment plate rides with shaft top
            _move(self._plate_ids[i], [ACT_X, y, shaft_top])

            # force arrows at shaft top
            base   = [ACT_X, y, shaft_top]
            tip    = [ACT_X, y, shaft_top + arrow_len]
            key    = f'arr{i}'
        ...
        # person + harness ride upward with the actuators
        self._person.update(ext)
